...
BX24.getFromTextarea = function(cb)
{
	sendMessageV2({ cmd: 'getFromTextarea', params: {}, cb });
};

BX24.insertInTextarea = function(text, cb)
{
	console.log('insertInTextarea hi')
	function encodeBase64(str) {
		return btoa(encodeURIComponent(str));
	}

	const payload = {text:"ололоgfkh;: :::/****/*/gfdgfd{gfdgd}", command: 'insertInTextarea'};
	const encoded = encodeBase64(payload);
	//sendMessage('insertInTextarea', encoded, cb);
	top.postMessage(payload, 'http'+(PARAMS.PROTOCOL?'s':'')+'://' + PARAMS.DOMAIN);
};
...
var sendMessage = function(cmd, params, cb)
{
	...
	//getInterface::349p1dfb8uchw5bi8j1i2oj5eleq51ve:b63caa3fe0d24bcaf7a0042e985863e1
...
const sendMessageV2 = function({ cmd, params, cb }) {
	const payload = {
		command: cmd,
		params,
		callbackId: setCallbackV2(cb),
...
const setCallbackV2 = function(cb) {
	if (cb && util.type.isFunction(cb)) {
		const cbId = util.uniqid();
		callbacks[cbId] = cb;
...
const runCallbackV2 = function(e) {
	e = e || window.event;
	const data = e.data;
	const expectedOrigin = 'http' + (PARAMS.PROTOCOL ? 's' : '') + '://' + PARAMS.DOMAIN;
	const isNotExpectedOrigin = e.origin !== expectedOrigin;
	const isInvalidData = !data || typeof data !== 'object';
	if (isNotExpectedOrigin || isInvalidData)
	{
		return;
	}

	const { callbackId, result } = data;
	const hasNoCallbacks = !callbackId || !callbacks[callbackId];
	if (hasNoCallbacks)
	{
		return;
	}
...
function handleMessage(e) {
	...
	const data = e.data;
	const isV1Message = util.type.isString(data);
	if (isV1Message)
	{
		sendMessage.runCallback(e);
	}
	else
	{
		runCallbackV2(e);
...
//BX24.bind(window, 'message', sendMessage.runCallback);
	BX24.bind(window, 'message', handleMessage);
